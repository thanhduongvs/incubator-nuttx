...
static uint32_t CLOCK_GetPeriphClkFreq(void)
{
    ...
    uint32_t reg = getreg32(IMXRT_CCM_CBCDR);
    if ((reg & CCM_CBCDR_PERIPH_CLK_SEL_MASK) != 0U)
    {
        reg = getreg32(IMXRT_CCM_CBCMR);
        switch (reg & CCM_CBCMR_PERIPH_CLK2_SEL_MASK)
        {
        ...
        freq /= (((reg & CCM_CBCDR_PERIPH_CLK2_PODF_MASK) >> CCM_CBCDR_PERIPH_CLK2_PODF_SHIFT) + 1U);
    }
    /* Pre_Periph_clk ---> Periph_clk */
    else
    {
        reg = getreg32(IMXRT_CCM_CBCMR);
        switch (reg & CCM_CBCMR_PRE_PERIPH_CLK_SEL_MASK)
        {
            ...
            case CCM_CBCMR_PRE_PERIPH_CLK_SEL(3U):
                ...
                freq = CLOCK_GetPllFreq(kCLOCK_PllArm) /
                       (((reg & CCM_CACRR_ARM_PODF_MASK) >> CCM_CACRR_ARM_PODF_SHIFT) + 1U);
                break;
...
uint32_t CLOCK_GetSemcFreq(void)
{
    ...
    uint32_t reg = getreg32(IMXRT_CCM_CBCDR);
    if ((reg & CCM_CBCDR_SEMC_CLK_SEL) != 0U)
    {
        /* PLL3 PFD1 ---> SEMC alternative clock ---> SEMC Clock */
        if ((reg & CCM_CBCDR_SEMC_ALT_CLK_SEL) != 0U)
        {
    ...
    freq /= (((reg & CCM_CBCDR_SEMC_PODF_MASK) >> CCM_CBCDR_SEMC_PODF_SHIFT) + 1U);
